...
while($operation){
    ...
    elseif($choice -eq 3){ 
        ...
        $checkingUser = checkUser($name)
        $checkingPassword = checkPassword($password)
        if($checkingUser -eq $true)
        {
            Write-Host "Username already exists"
        }
        else{
            if($checkingPassword -eq $true){
                createAUser $name $password
            
                Write-Host "User: $name is created." | Out-String
            }
            else{
                Write-Host 'Password does not meet the requirement of 1 special character, 1 number, at least 6 characters long'
            }
        }
    }
    ...
    elseif($choice -eq 4){
        ...
        $checkingUser = checkUser($name)
        if($checkingUser -eq $false)
        {
            Write-Host "Username does not exsist"
        }
        else
        {
            removeAUser $name
    ...
    elseif($choice -eq 7){
        ...
        {
            $numofDays = Read-Host -Prompt 'Enter the number of day you would like to check logs for'
            $userLogins = getLogInAndOffs $numofDays
    ...
    elseif($choice -eq 8){
        ...
        {
            $numofDays = Read-Host -Prompt "Enter Num of days you want to check failed logs for"
            $userLogins = getFailedLogins $numofDays
            # TODO: Change the above line in a way that, the days 90 should be taken from the user
    ...
    elseif($choice -eq 9){
        $numofDays = Read-Host -Prompt "Please enter the number of days you want to check for users at risk"

        $userfailedLogs = getFailedLogins $numofDays

        $userfailedLogs  | Select-Object User | Group-Object User -NoElement
    }
